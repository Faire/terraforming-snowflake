...
from resources import (
    SnowflakeDatabase, SnowflakeStage, SnowflakeWarehouse, SnowflakeRole,
    SnowflakeSchema, SnowflakePipe, SnowflakeFileFormat
)
...
def tf_databases(t):
    ...
    # snowflake query_to_df fails for unknown reason, doing it the old fashion way
    ...
    for row in db_dicts:
        ...
        tfDatabase.append_import_command_to_file(filename='generated_tf_snowflake_import_resources.sh')
...
def tf_schemas(t, database_names):
    ...
    for db in database_names:
        # NOTE: We may want to separate tf files by database
        schema_data = snowflake_client.exec_sql_multi(f"show schemas in database {db}")
        columns = ['created_on','name','is_default','is_current','database_name','owner','comment','options','retention_time']
        schema_dicts = [{k: row[i] for i, k in enumerate(columns)} for row in schema_data]
        ...
        for schema in schema_dicts:
            ...
            if schema['database_name'] == "RAW":
                if any([x in schema['name'] for x in ["PUBLIC", "KINESIS_", "CHARM_EXTERNAL"]]):
                    ...
                    tfSchema.append_import_command_to_file(filename='generated_tf_snowflake_import_resources.sh')
            else:
...
def tf_stages(t, database_names):
    ...
    #  and grabbing the `stage` table.
    # NOTE: We probably want to avoid special autoschemas, like `information_schema`? not implemented currently
    database_names = ['RAW']
    for database in database_names: 
        # NOTE: We may want to separate schema.tf files by database? not implemented currently
        columns = ['created_on','name','database_name','schema_name','url',
        ...
        for row in stage_dicts:
            stage_extra_data = snowflake_client.exec_sql_multi(f"desc stage {row['schema_name']}.{row['name']}")
            ...
            tfStage.append_import_command_to_file(filename='generated_tf_snowflake_import_resources.sh')
...
def tf_file_format(t, database_names):
    ...
    for database in database_names:
        ...
        columns = ['created_on','name','database_name','schema_name','type','owner','comment','format_options']
        file_format_dicts = [{k: row[i] for i, k in enumerate(columns)} for row in file_format_data]
        for row in file_format_dicts:
            ...
            tfFileFormat.append_import_command_to_file(filename='generated_tf_snowflake_import_resources.sh')
...
def tf_warehouses(t):
    ...
    columns = ['name','state','type','size','min_cluster_count','max_cluster_count',
        ...
        'resumed_on','updated_on','owner','comment','resource_monitor','actives',
        'pendings','failed','suspended','uuid','scaling_policy']
    wh_dicts = [{k: row[i] for i, k in enumerate(columns)} for row in wh_data]
    for row in wh_dicts:
        addtl_params = snowflake_client.exec_sql_multi(f"show parameters in warehouse {row['name']};")
        cols = ['key','value','default','level','description','type']
        addtl_params_dict = {row[0].lower():{k: row[i] for i, k in enumerate(cols)}['value'] for row in addtl_params}
        row.update(addtl_params_dict)
        tfDatabase = SnowflakeWarehouse(
            attr_exclusion_rules=attr_exclusion_rules,
            regex_exclusion_rules=regex_exclusion_rules,
            **row)
        tfDatabase.append_tf_code_to_file(t.working_dir)
        tfDatabase.append_import_command_to_file(filename='generated_tf_snowflake_import_resources.sh')
...
def tf_roles(t):
    ...
    for row in role_dicts:
        ...
        tfRole.append_import_command_to_file(filename='generated_tf_snowflake_import_resources.sh')
...
def tf_pipes(t, database_names):
    ...
    for database in database_names:
        ...
        for _, row in pipe_data.iterrows():
            ...
            tfPipe.append_import_command_to_file(filename='generated_tf_snowflake_import_resources.sh')
